...
+ (void)bookDetailsByBookId:(NSString *)bookid
                     ...
                     withBlock:(void (^)(BOOL success, Book *obj,NSError *error))block;
...
+ (void)bookCatalogue:(NSString *)cataid VIP:(BOOL)VIP
            withBlock:(void(^)(BOOL success, NSString *content, NSString *message, NSError *error))block; //内容 提示语 提示code
...
//默认推荐书籍
+ (void)recommandDefaultBookwithBlock:(void(^)(BOOL success, NSArray *resultArray,NSError *error))block;
